...
- **Bearer-token authentication required on all endpoints** (`agent_brain_server/api/security.py`, `agent_brain_server/api/main.py`, `agent_brain_server/config/settings.py`, `agent_brain_server/runtime.py`, `agent_brain_cli/client/api_client.py`, `agent_brain_cli/client/transport.py`, `agent_brain_cli/config.py`, `agent_brain_cli/commands/init.py`, `agent_brain_cli/commands/start.py`, `agent_brain_cli/commands/stop.py`, `.env.example`, `README.md`). Previously every endpoint was unauthenticated: on the multi-agent same-box deployment pattern, any local process could read the entire index via `POST /query` or wipe it via `DELETE /index` — `127.0.0.1` is not an authorization boundary (OWASP API1:2023 BOLA + API2:2023 Broken Auth). Every router except `/health` now requires `Authorization: Bearer <key>`, verified by `verify_bearer_token` with constant-time comparison; missing/malformed/wrong credentials return 401, and a misconfigured server (auth required but no key set) fails closed with 503. Secure by default: `agent-brain init` auto-generates a 32-byte url-safe key into `.agent-brain/runtime.json` (mode 600), and the server backfills one on first start if absent, so existing installs upgrade transparently; the server refuses to start without a key unless `agent-brain-serve --insecure` / `agent-brain start --insecure` is passed (with a loud startup warning), mirroring how Postgres requires `--allow-empty-password` and Redis `protected-mode no` to run unauthenticated. The CLI reads the key from runtime.json and sends it on every request, so single-user dev stays frictionless. The `Authorization: Bearer` scheme (rather than `X-API-Key`) is forward-compatible with the planned MCP v4 OAuth 2.1 work (#188) — one scheme that evolves from a static key to OAuth-issued bearer tokens. `/docs`, `/redoc`, and `/openapi.json` are served only when `DEBUG=true` so shared/production hosts do not leak their API surface. `agent-brain stop` reports "No server running" for a PID-less runtime.json (the init-written key stub). 17 tests in `tests/test_api_auth.py` cover every dependency branch, the HTTP 401/200 matrix, insecure-mode bypass, key generation + runtime.json mode-600 persistence, and refuse-to-start; 7 tests in `agent-brain-cli/tests/test_api_key_auth.py` pin CLI key resolution (env > runtime.json > none) and bearer-header attachment. Closes #179.
