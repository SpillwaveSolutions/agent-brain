...
- **`JwksTokenVerifier` / `LocalRs256Verifier` no longer require the optional `nbf` claim** (`agent_brain_mcp/oauth/verifier.py`; addresses [#218](https://github.com/SpillwaveSolutions/agent-brain/issues/218)). RFC 7519 marks `nbf` optional; Keycloak access tokens typically omit it. `nbf` is still verified when present. The Keycloak JWT assertion now dumps unverified `iss`/`aud`/`azp`/`nbf` so the next CI run settles remaining claim mismatches with evidence.
