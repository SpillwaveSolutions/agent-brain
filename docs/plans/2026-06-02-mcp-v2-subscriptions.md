...
#### 3.2.1 Phase 51 ship outcome (updated 2026-06-03 after Plan 04 close)

All four plans landed in a single working day (Plans 01-03 in parallel waves, Plan 04 as the finalization wave). Key outcomes the v2 design now depends on:

- **Forward-compatibility commitment:** The four `uriTemplate` strings above are now published by `@server.list_resource_templates()` in `agent_brain_mcp/server.py`. Once a release ships with these strings, downstream MCP client libraries lock onto them. Changing any of them after 10.2.0 ships is a breaking change requiring a new major-version coordination with Anthropic Claude Code, OpenCode, and any third-party MCP clients keying off the templates.
- **`{+path}` operator choice (decided in Plan 04):** the `file` template uses RFC 6570 *reserved expansion* (`{+path}`) rather than the default expansion (`{path}`). Default expansion percent-encodes `/` as `%2F`, which is the wrong behavior for filesystem paths — MCP clients expanding the template to `file:///tmp/foo.py` need the slashes preserved. Plan 04 includes a test (`test_file_template_uses_reserved_expansion`) that pins this choice so a future "simplify the template" PR cannot silently regress it.
- **`mimeType` policy:** the three JSON-backed schemes (`chunk`, `graph-entity`, `job`) advertise `application/json` statically. `file://` deliberately omits `mimeType` — the type is sniffed per-read via `mimetypes.guess_type`, and advertising a single static MIME on the template would misroute file-type detection at the client side.
- **Release-train coupling (Plan 04 enforces):** the runtime version-floor check (`check_backend_version`) now refuses to start against `agent-brain-server < 10.2.0`. The matching install-time pin in `agent-brain-mcp/pyproject.toml` (`agent-brain-rag = "^10.2.0"`, `agent-brain-uds = "^10.2.0"`) was bumped in the same commit so both checks agree. Release ordering is locked: `agent-brain-server 10.2.0` MUST publish to PyPI BEFORE `agent-brain-mcp 10.2.0`. If MCP ships first, operators upgrading get a clean refusal-to-start error rather than confusing 404s; if both ship close together, the existing lockstep release flow (`.claude/commands/ag-brain-release.md`) handles ordering.
- **#178 Kuzu SIGSEGV carry-forward:** the `graph-entity://` handler preserves the Phase 50 503 `detail.error` slug (`graphrag_disabled` or `kuzu_unavailable`) and exposes it as `data.reason` on the resulting `McpError`. Operators hitting #178 mid-flight can keep the MCP surface responsive by setting `graphrag.store_type: simple` in the server config (use `SimplePropertyGraphStore` instead of Kuzu) until #178 is fixed. MCP clients can route on `data.reason` without re-parsing cause strings.
- **`resources.subscribe` capability stays `False`** through Phase 51. Phase 52 owns flipping it; Plan 04 deliberately does NOT touch `get_capabilities()` so the v1 wire shape is preserved for clients that don't yet understand subscriptions.
- **Tests added in Plan 04:** 13 in `test_resources_templates_list.py` (registry-level + handler-level + corpus://* regression) plus 8 fresh cases in `test_version_compat.py` pinning the 10.2.0 floor, plus 1 e2e SDK test (`test_e2e_templates_list_and_read_all_schemes`) that drives all four schemes through the real MCP wire protocol. Combined: Phase 51 ships with 31 net new tests across the four plans.
...
#### 3.3.1 Phase 52 ship outcome (updated 2026-06-03 after Plan 04 close)

All four plans landed across a single working day (Plan 01 — manager core, Plan 02 — wire handlers + capability flip, Plan 03 — per-URI policies + `SubscriptionTerminated` sentinel, Plan 04 — disconnect cleanup + SDK e2e tests). Key outcomes the v2 design now depends on:

- **Subscribable URI allowlist enforced** (`SUB-01`, `SUB-02`, `SUB-03`). Three policies live in `agent_brain_mcp/subscriptions/policies.py`:
  - `JobPolicy` (scheme key `"job://"`, 1.0s cadence) — `_build_job_fetcher` returns a closure that `await asyncio.to_thread(api.get_job, id)` and raises `SubscriptionTerminated(payload)` when `status in {completed, failed, cancelled}`. The polling loop emits one final `on_change` with the terminal payload, then exits cleanly via Plan 01's synchronous-cleanup paths.
  - `CorpusStatusPolicy` (exact key `"corpus://status"`, 30.0s cadence) — `drop_keys = DEFAULT_DROP_KEYS | {"request_id"}` so uvicorn's per-request UUID doesn't spuriously bump the SHA-256 every 30s.
  - `CorpusFoldersPolicy` (exact key `"corpus://folders"`, settings-driven default 5.0s) — `drop_keys = DEFAULT_DROP_KEYS | {"last_polled"}` PRESERVING `last_indexed` (file watcher + index job completion update it; it's a real change signal). The 60s "safety poll" cadence (`folders_safety_interval_s`) is a settings field but NOT wired through the loop in v2 — reserved for a v3 micro-plan if the 5s active cadence proves insufficient.
- **Two-step subscribable-URI gate** at the wire handler: (1) `_is_known_uri` rejects URIs outside `RESOURCE_REGISTRY` + `PARAMETERIZED_SCHEMES` with `SubscribableUriRejected(reason="unknown_uri")`; (2) `resolve_policy` returns `None` for known-but-non-subscribable URIs (`corpus://config`, `chunk://`, `graph-entity://`, `file://`) → `reason="not_subscribable"`. Duplicate subscribe (same session, same URI) is pre-checked via `manager.is_subscribed()` → `reason="duplicate_subscribe"`. All three reasons surface in `McpError.data.reason` so clients can route without re-parsing message text.
- **Capability flip via `get_capabilities` wrapper** (Plan 02). MCP SDK 1.12.x hardcodes `resources.subscribe=False` at `mcp/server/lowlevel/server.py:211` with no opt-in knob on `NotificationOptions` and no derivation from `_subscribe_resource_handler` presence. `build_server()` installs a `_patched_get_capabilities` wrapper that calls the original and flips `caps.resources.subscribe = True`. Documented inline with a line-number reference; future SDK upgrades can drop the wrapper if/when the capability becomes flag-driven. `caps.resources.listChanged` remains `False` — `listChanged` is out of scope for v2 (no `resources/listChanged` notification pipeline).
- **`build_server()` tuple return shape** (Plan 04 refactor). `build_server(httpx_client, *, transport="http")` now returns `tuple[Server, SubscriptionManager]`. Plan 02's `server._subscription_manager` private-attr workaround is preserved for backwards compatibility (the same `SubscriptionManager` instance is attached as both the tuple's second element AND the private attr) but new callers should prefer unpacking. Tests pin the identity-equality contract.
- **`run_stdio()` disconnect cleanup hook** (Plan 04, `SUB-05`). The body of `run_stdio(server, subscription_manager)` is wrapped in `try / finally`; the `finally` calls `subscription_manager.cleanup_all()` on EVERY exit path (graceful stdio EOF, exception, mid-loop crash). `cleanup_all()` is idempotent (empty registry returns 0) so re-entrancy is safe. The HTTP transport analog is Phase 53's responsibility — Phase 52 documents the contract here so Phase 53 inherits it verbatim.
- **`_poll_loop` defense-in-depth** (Plan 04). The polling loop now has an explicit `except asyncio.CancelledError` clause that DEBUG-logs the session/uri then re-raises. Plan 01's `finally` block already scrubs the registry slot; the explicit catch makes Plan 04's leaked-task assertion test diagnosable in CI without ratcheting the logger to DEBUG for every test.
- **SDK e2e suite landed** (`tests/e2e/test_e2e_subscriptions.py`, 5 tests). Spawns the MCP server as a subprocess via the official `mcp.client.stdio.stdio_client` + `mcp.ClientSession`, registers a `message_handler` callback that collects `ResourceUpdatedNotification` instances off the SDK's server-notification stream, and exercises:
  1. `corpus://status` emits ≥1 notification at fast cadence (`STATUS_INTERVAL_S=0.3`) within 1.5s.
  2. `job://job-fast` running → running → completed emits ≥2 notifications; unsubscribe after terminal is a no-op (slot already scrubbed via `SubscriptionTerminated`).
  3. `corpus://folders` 2-state flip emits ≥2 distinct notifications within 1.5s.
  4. **`test_disconnect_cleans_up_polling_tasks`** — the load-bearing assertion for `SUB-05`. Subscribes; counter-based proof of polling activity; exits both `ClientSession` and `stdio_client` contexts; waits 1.5s; asserts the counter delta is ≤ 1 (allows for one in-flight poll that crossed the cancellation boundary). Deterministic — no psutil thread-count dependence. The SDK's `stdio_client` task group surfaces an `ExceptionGroup` containing `anyio.BrokenResourceError` when the subprocess emits a final in-flight notification after the client closed its read side; this is harmless (the subprocess write succeeds; the parent stopped reading) and the test catches the group, filters out `BrokenResourceError`, and re-raises any other exception so real bugs aren't swallowed.
  5. Two-sessions cross-process isolation. For stdio, each `stdio_client` invocation spawns its own MCP subprocess; closing session A's subprocess must not affect session B's notification stream. The "real" same-process-multi-session isolation is Plan 01's `test_two_sessions_for_same_uri_get_independent_tasks` which runs inside a single asyncio loop.
- **Notification payload conformance** (`SUB-04`). `tests/test_notification_shape.py` pins both the minimal URI-only shape (the Phase 52 default that `ServerSession.send_resource_updated(uri)` produces) and the optional `_meta.revision` envelope (CONTEXT decision C "when known" path) against the SDK's `ResourceUpdatedNotification` / `ResourceUpdatedNotificationParams` Pydantic models. The revision string MUST be a 64-char hex SHA-256 of the canonical payload — independently verified against `hashlib.sha256(json.dumps(stripped, sort_keys=True, separators=(",", ":")))` so future digest-computation drift trips the test.
- **`SUBSCRIPTION_POLICIES` registry key shape:** either an exact URI string (`"corpus://status"`) OR a scheme prefix ending in `"://"` (`"job://"`). `resolve_policy` tries exact first, then scheme-prefix — exact entries always win over scheme entries. Critical for any future registry growth where an exact URI must override a scheme default.
- **Operator settings (Plan 03):** two new env vars — `AGENT_BRAIN_MCP_SUBSCRIPTION_FOLDERS_ACTIVE_INTERVAL_S` (default 5.0) and `AGENT_BRAIN_MCP_SUBSCRIPTION_FOLDERS_SAFETY_INTERVAL_S` (default 60.0). `MCPSubscriptionSettings` Pydantic model with `Field(default=..., gt=0)` validation; non-positive intervals raise `RuntimeError` at config-module import with the offending env var name surfaced. No hot reload — restart the MCP server to pick up changes.
- **Test count delta:** +94 net new tests across the four plans (Plan 01 +39 unit + manager; Plan 02 +26 wire + capability + e2e; Plan 03 +40 per-URI policies + integration; Plan 04 +14 — 5 e2e SDK + 9 notification shape conformance + test_initialize tuple-shape update). MCP package goes from 141 (post-Phase 51) to 265 + 28 skipped (full suite, `pytest -m ''`). The Plan 02 + Plan 03 + Plan 04 e2e suites together exercise 15 e2e SDK tests against the real MCP wire.
- **Reuse contract for Phase 54 TOOL-04 (`wait_for_job`) locked:** `SubscriptionManager.start_polling(session, uri, interval_s, fetcher, on_change, drop_keys=None)` signature is now LOCKED. `wait_for_job` will import it verbatim and raise `SubscriptionTerminated(final_progress_payload)` from its one-shot polling fetcher when the job reaches a terminal status. Phase 54 does NOT need to coordinate with Phase 52 re: signature changes — any future evolution of the polling primitive is a cross-phase breaking change.
- **Risk register carry-forward to Phase 53:**
  - The disconnect-cleanup contract Plan 04 ships for stdio (`try/finally` in `run_stdio` calling `cleanup_all()` on every exit path) is the template for Phase 53's HTTP analog. Streamable HTTP transport will need an equivalent hook in its session-manager lifecycle so HTTP-disconnected sessions don't leak polling tasks. The risk note "subscription leak in HTTP transport" (§4) is now scope-committed for Phase 53 Plan 01.
  - The SDK's `BrokenResourceError` filtering pattern in `test_disconnect_cleans_up_polling_tasks` may need to extend to HTTP-transport e2e tests in Phase 53. If the HTTP framing layer raises a different exception type on disconnect, Phase 53 inherits the filter-pattern with a different exception class.
