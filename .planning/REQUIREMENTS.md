...
- [x] **OAUTH-07**: `McpHttpBackend` handles the 401 + `WWW-Authenticate` challenge and the full OAuth dance via the SDK `OAuthClientProvider`, persisting tokens in a `FileTokenStorage` keyed to `state_dir` so per-call (Pattern A) invocations reuse the token instead of re-triggering the browser dance.
