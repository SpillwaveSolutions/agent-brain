...
4. **EntityType** — Combined Literal of all three categories (all 18 values in one Literal)
...
def normalize_entity_type(raw_type: str | None) -> str | None:
    ...
    Handles case normalization and AST symbol type mapping.
    Returns None if input is None, returns original string if no mapping found.
    """
    if raw_type is None:
        return None
    mapped = SYMBOL_TYPE_MAPPING.get(raw_type.lower())
    if mapped:
        return mapped
    # Try PascalCase normalization
    if raw_type.capitalize() in ENTITY_TYPES:
...
assert normalize_entity_type('CLASS') is not None
...
```
  ...
  <done>
EntityType, RelationshipType Literal types defined with all 18 entity types and 8 relationship predicates. SYMBOL_TYPE_MAPPING and normalize_entity_type helper exist. GraphTriple backward compatibility preserved (subject_type/object_type remain str | None). All types exported from models/__init__.py.
  </done>
</task>
...
1. `test_entity_types_complete` — verify ENTITY_TYPES has exactly 18 entries and contains expected values from each category
...
7. `test_normalize_entity_type_case_insensitive` — verify normalize_entity_type("CLASS") returns "Class" (via capitalize check)
