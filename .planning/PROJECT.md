...
**Current State (v10.4 IN PROGRESS — Phase 66 complete 2026-06-14):**
- v10.4 MCP v4 (OAuth 2.1) underway. Phase 65 filed + security-gated the OAuth design doc (OAUTH-01). **Phase 66 complete**: the public OAuth discovery root is live — PRM (RFC 9728) + OASM (RFC 8414, `S256`) served unauthenticated at the `.well-known` paths and mounted ABOVE the `/mcp` Mount (mount-order contract proven to survive Phase 67's middleware), and the `AGENT_BRAIN_AUTH` settings toggle + boot startup gate (exit-2 on invalid/oauth-misconfig) are wired (OAUTH-02/03/09; 109 phase tests, 89% MCP coverage). No token issuance / request-path enforcement yet — that is Phase 67 (co-located AS+RS middleware). Next: Phase 67.
