...
- [ ] **CLI-MCP-05..07**: CLI prompts (`agent-brain prompt <name>`), resources (`agent-brain resources list|read <uri>`) — Phase 59 (CLI-MCP-01..04 validated in Phases 56-57; CLI-MCP-08..10 validated in Phase 58)
...
---
*Last updated: 2026-06-07 — Phase 58 (Runtime discovery + helper commands) COMPLETE. Validated CLI-MCP-08, CLI-MCP-09, CLI-MCP-10 (3/3 plans, 8/8 must-haves). `mcp.runtime.json` schema + `agent-brain mcp start|stop` Click subcommands wired; `McpHttpBackend` auto-discovers `host:port` via the runtime file when `--mcp-url` is omitted; Phase 57 placeholder error string deleted and replaced with verbatim §3.5 wording at three surfaces. End-to-end integration proves the full `start → query without --mcp-url → stop` cycle. Latent pre-existing `--url envvar=AGENT_BRAIN_URL` quirk surfaced — fix point is Phase 60 env allowlist. Next: Phase 59 (CLI prompts + resources commands) — `agent-brain prompt <name>` + `agent-brain resources list|read <uri>` surface MCP prompts and resources to humans.*
