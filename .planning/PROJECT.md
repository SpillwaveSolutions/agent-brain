...
v10.3 shipped 2026-06-14. Candidates for the next milestone (run `/gsd:new-milestone` to scope):

- [ ] **OAUTH-01..N**: OAuth 2.1 for the Streamable HTTP transport (PRM, DCR, Resource Indicators, optional DPoP) — depends on v10.3's `McpHttpBackend`. Issue [#188](https://github.com/SpillwaveSolutions/agent-brain/issues/188). Strongest v10.4 candidate.
- [ ] **FRAME-OPT-01..05**: Promote the 5 docs-only editor recipes (Goose, Continue.dev, Cline, Cursor, Cody) to smoke-test parity if adoption justifies it.
- [ ] **RUNTIME-PARITY-47/48/49**: Headless project-local install + execution verification for Codex / OpenCode / Gemini (parked from v9.6.0; re-evaluate now that the framework matrix exercises external CLIs).
